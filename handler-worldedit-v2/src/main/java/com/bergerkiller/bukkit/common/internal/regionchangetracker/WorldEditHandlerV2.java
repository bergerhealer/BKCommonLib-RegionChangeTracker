...
final class WorldEditHandlerV2 implements RegionChangeTrackerHandler {
    ...
    @Override
    public String name() {
        return "WorldEdit (>= v7.0.0)";
    }

    @Override
    public HandlerInstance<?> enable(RegionChangeTrackerHandlerOps ops) throws Exception, Error {
        return new WEV2HandlerInstance(this, ops);
    }

    private static final class WEV2HandlerInstance extends HandlerInstance<WorldEditHandlerV2> {
        private final Method adaptWorldMethod;
        private final EventBus eventBus;

        public WEV2HandlerInstance(WorldEditHandlerV2 handler, RegionChangeTrackerHandlerOps ops) throws Exception {
            super(handler, ops);
